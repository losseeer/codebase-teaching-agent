...
/**
  代码地图的 LLM 完善层（用户可见的命名/摘要质量，结构仍由静态分析锚定）：
...
export async function refineCourseMap(tree: CourseTree, provider: LlmProvider): Promise<CourseMapRefinement> {
  try {
    ...
    const system = [
      ...
      `为其中值得润色的节点重写中文标题（≤${MAX_TITLE} 字）与摘要（≤${MAX_SUMMARY} 字）。`,
      "",
      ...
      "- overview/workflow 用动作短语概括链路（如「从入口到路由的启动链路」）；module 用名词短语点明职责（如「配置加载与校验」）。",
      "- 摘要规则：只陈述能从路径、锚点与现有摘要推断的事实（做什么、被谁调用、依赖什么）；不评价代码质量、不猜测作者动机、不编造未提供的细节。",
      "- 拿不准的节点直接省略它的 key（输出里不出现），宁缺毋滥；不要为了覆盖全部节点而编内容。",
      "",
