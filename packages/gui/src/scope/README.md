...
| 作用域 | 默认可见 | 动作触发的小字分隔线 | 触发的 Agent 气泡 |
|---|---|---|---|
| 宏观设计 | 依赖图 / 流程节点 / README 摘要 / 入口文件 | `已切换到 · 节点标题`、`已选中 · file` | 宏观结构与上下游 |
| 代码教学 | 当前源码 tab / 课程树 / 苏格拉底状态机上下文 | `已打开 · file`、`已定位 · file:line（标题）` | 苏格拉底式提问与回退 |
| 练习评估 | 评分标准 / 提示阶梯 / 历史错题 / ZPD 校准 | `模块 · A → B`、`练习 #N · 标题 → file` | 判分反馈 + 下一题建议 |
